...
def test_add_friend_not_existed():
	socnet = SocNet()
	assert socnet.add_friend(1, "friend_to_add") == "User is not existed"
...
def test_delete_non_existed_friend():
	socnet = SocNet()
	assert socnet.delete_friend(2, [4, "friend_to_del"]) == "User is not in friend list"
...
def test_get_list_friends():
	socnet = SocNet()
	assert socnet.get_list_friends(1) == [[3, "friend1"]]
